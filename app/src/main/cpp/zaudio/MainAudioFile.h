//
// Created by lap13663 on 8/9/20.
//

#ifndef AUDIOEXAMPLE_MAINAUDIOFILE_H
#define AUDIOEXAMPLE_MAINAUDIOFILE_H
...
void AudioFile<T>::printSummary() const {
    ...
    LOG_D("|======================================|\n");
    LOG_D("Num Channels: %d\n", getNumChannels());
    LOG_D("Num Samples Per Channel: %d\n", getNumSamplesPerChannel());
    LOG_D("Sample Rate: %zu\n", sampleRate);
    LOG_D("Bit Depth: %d\n", bitDepth);
    LOG_D("Length in Seconds: %f\n", getLengthInSeconds());
    LOG_D("|======================================|\n");
}
...
void AudioFile<T>::reportError(std::string errorMessage) {
    if (logErrorsToConsole)
        LOG_E("Error: %s", errorMessage.c_str());
}
...
#endif //AUDIOEXAMPLE_MAINAUDIOFILE_H
